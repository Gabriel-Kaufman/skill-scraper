...
import type { Skill, TrackedSkill, SurgeAlert, State } from "./types.js";

function toTracked(skill: Skill): TrackedSkill {
  return {
    slug: skill.slug,
...
function detectSurges(current: TrackedSkill[], prevState: State): SurgeAlert[] {
...
function getTopMovers(current: TrackedSkill[], prevState: State, count: number): Mover[] {
  const movers: Mover[] = [];

  for (const skill of current) {
    const prev = prevState.skills[skill.slug];
    if (!prev || prev.downloads === 0) continue;
...
function formatSurgeAlert(alert: SurgeAlert): string {
...
function formatTopMovers(movers: Mover[]): string {
  if (movers.length === 0) return "  (no movement data yet — run check again after some time)\n";
  return movers
    .map((m, i) =>
      `  ${i + 1}. ${m.skill.displayName.padEnd(35)} ` +
...
export async function runCheck(): Promise<SurgeAlert[]> {
