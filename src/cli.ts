...
async function cmdStatus() {
  ...
  console.log(`\n  Top 5 by downloads:`);
  top5.forEach((s, i) =>
    console.log(`    ${i + 1}. ${s.displayName.padEnd(35)} DL: ${s.downloads.toLocaleString().padStart(8)}  ★ ${s.stars}`)
  );
}
...
async function cmdConfig(args: string[]) {
  ...
  for (const arg of args) {
    ...
    switch (key) {
      case "interval":   config.intervalHours = parseInt(val); break;
      case "downloads":  config.thresholds.minDownloads = parseInt(val); break;
      case "stars":      config.thresholds.minStars = parseInt(val); break;
      case "growth":     config.thresholds.minGrowthPct = parseInt(val); break;
      case "movers":     config.topMovers.count = parseInt(val); break;
      case "movers-off": config.topMovers.enabled = false; break;
      case "movers-on":  config.topMovers.enabled = true; break;
...
async function cmdFetch() {
  ...
  skills.slice(0, 20).forEach((s, i) => {
    console.log(
      `${String(i + 1).padStart(2)}. ${s.displayName.padEnd(35)} ` +
...
switch (command) {
  ...
  case "check":
  case "manual":  await runCheck(); break;
