import type { Skill, SkillsResponse } from "./types.js";

const API_BASE = "https://clawhub.ai/api/v1";
const PAGE_LIMIT = 50;

async function fetchPage(cursor?: string): Promise<SkillsResponse> {
...
export async function fetchTrending(maxPages = 4): Promise<Skill[]> {
  const all: Skill[] = [];
